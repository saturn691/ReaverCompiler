/*!
    ...
    The main purpose of the library is to implement simple yet flexible C/C++
   preprocessor. We've hardly tried to stick to C98 preprocessor's
   ...
   implementation of preprocessor for GLSL and HLSL languages.

    The usage of the library is pretty simple, just copy this file into your
   enviornment and predefine TCPP_IMPLEMENTATION macro before its inclusion like
   the following
...
public:
    ...
    /*!
        ...
        \param[in] offset Distance of overlooking, passing 0 is equivalent to
       invoking GetNextToken()
    */
...
public:
    ...
    using TOnIncludeCallback =
        std::function<TInputStreamUniquePtr(const std::string &, bool)>;
    using TSymTable = std::vector<TMacroDesc>;
    using TContextStack = std::list<std::string>;
    using TDirectiveHandler = std::function<
        std::string(Preprocessor &, Lexer &, const std::string &)>;
    ...
    typedef struct TPreprocessorConfigInfo
    {
        ...
        bool mSkipComments = false; ///< When it's true all tokens which are
                                    ///< E_TOKEN_TYPE::COMMENTARY will be thrown
                                    ///< away from preprocessor's output
...
public:
    ...
    Preprocessor(Lexer &lexer, const TPreprocessorConfigInfo &config)
        TCPP_NOEXCEPT;
    ~Preprocessor() TCPP_NOEXCEPT = default;

    bool AddCustomDirectiveHandler(
...
private:
    ...
    std::vector<TToken> _expandMacroDefinition(
        const TMacroDesc &macroDesc,
        const TToken &idToken,
        const std::function<TToken()> &getNextTokenCallback) const
        TCPP_NOEXCEPT;

    void _expect(
        const E_TOKEN_TYPE &expectedType,
...
std::string ErrorTypeToString(const E_ERROR_TYPE &errorType) TCPP_NOEXCEPT
{
    ...
    case E_ERROR_TYPE::INCONSISTENT_MACRO_ARITY:
        return "Inconsistent number of arguments between definition and "
...
StringInputStream &
StringInputStream::operator=(const StringInputStream &stream) TCPP_NOEXCEPT
{
...
StringInputStream &
StringInputStream::operator=(StringInputStream &&stream) TCPP_NOEXCEPT
{
...
static std::tuple<size_t, char>
EatNextChar(std::string &str, size_t pos, size_t count = 1)
{
    str.erase(0, count);
...
static std::string
ExtractSingleLineComment(const std::string &currInput) TCPP_NOEXCEPT
{
...
static std::string ExtractMultiLineComments(
    std::string &currInput,
    const std::function<std::string()> &requestNextLineFunctor) TCPP_NOEXCEPT
{
    ...
    std::function<std::string(std::string &)> enterCommentBlock =
        [&enterCommentBlock, &commentStr, requestNextLineFunctor](
            std::string &input)
    {
...
TToken Lexer::_scanTokens(std::string &inputLine) TCPP_NOEXCEPT
{
    ...
    while (!inputLine.empty())
    {
        ...
        if (ch == '.' && PeekNextChar(inputLine, 1) == '.' &&
            PeekNextChar(inputLine, 2) == '.')
        {
            // flush current blob
            if (!currStr.empty())
            {
        ...
        if (ch == '/')
        {
                ...
                '/') // \note Found a single line C++ style comment
            {
                commentStr = ExtractSingleLineComment(inputLine);
                                                        ...
                                                        /// commentary
            {
                commentStr = ExtractMultiLineComments(
                    inputLine, std::bind(&Lexer::_requestSourceLine, this));
            }

            if (!commentStr.empty())
            {
                mCurrPos = std::get<size_t>(
                    EatNextChar(inputLine, mCurrPos, commentStr.length()));
                return {
                    E_TOKEN_TYPE::COMMENTARY,
                    commentStr,
                    mCurrLineIndex,
                    mCurrPos};
            }
        ...
        if (ch == '\n' || ch == '\r')
        {
            ...
            mCurrPos = std::get<size_t>(
                EatNextChar(inputLine, mCurrPos, separatorStr.length()));
            return {
                E_TOKEN_TYPE::NEWLINE, separatorStr, mCurrLineIndex, mCurrPos};
        }
        ...
        if (ch == '#') // \note it could be # operator or a directive
        {
            ...
            for (const auto &currDirective : mDirectivesTable)
            {
                ...
                if (inputLine.rfind(currDirectiveStr, 0) == 0)
                {
                    ...
                    return {
                        std::get<E_TOKEN_TYPE>(currDirective),
                        "",
                        mCurrLineIndex,
                        mCurrPos};
                }
            ...
            if (!inputLine.empty())
            {
                ...
                case '#': // \note concatenation operator
                    ...
                    ++mCurrPos;
                    return {
                        E_TOKEN_TYPE::CONCAT_OP, "", mCurrLineIndex, mCurrPos};
                default:
                    if (nextCh != ' ') // \note stringification operator
                    {
                        return {
                            E_TOKEN_TYPE::STRINGIZE_OP,
                            "",
                            mCurrLineIndex,
                            mCurrPos};
                    }
        ...
        if (std::isdigit(ch))
        {
            ...
            if (ch == '0' && !inputLine.empty())
            {
                ...
                {
                    return {
                        E_TOKEN_TYPE::NUMBER, number, mCurrLineIndex, mCurrPos};
                }
            }
        ...
        if (ch == '_' || std::isalpha(ch)) ///< \note parse identifier
        {
            ...
            return {
                (keywordsMap.find(identifier) != keywordsMap.cend())
...
static bool
IsEscapeSequenceAtPos(const std::string &str, std::string::size_type pos)
{
    ...
    static constexpr char escapeSymbols[]{
        '\'', '\\', 'n', '\"', 'a', 'b', 'f', 'r', 't', 'v'};
...
std::string Lexer::_requestSourceLine() TCPP_NOEXCEPT
{
    ...
    while (((pos = sourceLine.find_first_of('\\')) != std::string::npos) &&
           (std::isspace(PeekNextChar(sourceLine, pos + 1)) ||
            PeekNextChar(sourceLine, pos + 1) == EOF) &&
           !IsEscapeSequenceAtPos(sourceLine, pos))
    {
        if (pCurrInputStream->HasNextLine())
        {
            sourceLine.replace(
...
TToken
Lexer::_scanSeparatorTokens(char ch, std::string &inputLine) TCPP_NOEXCEPT
{
    switch (ch)
    {
    case ',':
    ...
    case '[':
        return {
            E_TOKEN_TYPE::OPEN_SQUARE_BRACKET, "[", mCurrLineIndex, mCurrPos};
    case ']':
        return {
            E_TOKEN_TYPE::CLOSE_SQUARE_BRACKET, "]", mCurrLineIndex, mCurrPos};
    case '<':
        if (!inputLine.empty())
        {
            char nextCh = inputLine.front();
...
Preprocessor::Preprocessor(Lexer &lexer, const TPreprocessorConfigInfo &config)
    TCPP_NOEXCEPT : mpLexer(&lexer),
                    mOnErrorCallback(config.mOnErrorCallback),
                    mOnIncludeCallback(config.mOnIncludeCallback),
                    mSkipCommentsTokens(config.mSkipComments)
{
...
bool Preprocessor::AddCustomDirectiveHandler(
    const std::string &directive,
    const TDirectiveHandler &handler) TCPP_NOEXCEPT
{
...
static bool inline IsParentBlockActive(
    const Preprocessor::TIfStack &conditionalsContext) TCPP_NOEXCEPT
{
    return conditionalsContext.empty()
               ? true
               : (conditionalsContext.top().mIsParentBlockActive &&
                  !conditionalsContext.top().mShouldBeSkipped);
}
...
std::string Preprocessor::Process() TCPP_NOEXCEPT
{
    ...
    while (mpLexer->HasNextToken())
    {
        ...
        case E_TOKEN_TYPE::IDENTIFIER: // \note try to expand some macro here
        {
            ...
            {
                mpLexer->AppendFront(_expandMacroDefinition(
                    *iter,
                    currToken,
                    [this] { return mpLexer->GetNextToken(); }));
            }
            else
            {
        ...
        case E_TOKEN_TYPE::CONCAT_OP:
            while (processedStr.back() ==
                   ' ') // \note Remove last character in the processed source
                        // if it was a whitespace
            {
                processedStr.erase(processedStr.length() - 1);
            }

            while ((currToken = mpLexer->GetNextToken()).mType ==
                   E_TOKEN_TYPE::SPACE)
        ...
        case E_TOKEN_TYPE::STRINGIZE_OP:
        {
            ...
            {
                mOnErrorCallback(
                    {E_ERROR_TYPE::INCORRECT_OPERATION_USAGE,
                     mpLexer->GetCurrLineIndex()});
                continue;
            }

            appendString(
                "\"" + (currToken = mpLexer->GetNextToken()).mRawView + "\"");
        }
        break;
        case E_TOKEN_TYPE::CUSTOM_DIRECTIVE:
        {
            ...
            {
                mOnErrorCallback(
                    {E_ERROR_TYPE::UNDEFINED_DIRECTIVE,
                     mpLexer->GetCurrLineIndex()});
            }
        }
        break;
        default:
            if (E_TOKEN_TYPE::COMMENTARY == currToken.mType &&
                mSkipCommentsTokens)
            {
                break;
            }
...
void Preprocessor::_createMacroDefinition() TCPP_NOEXCEPT
{
    ...
    auto extractValue = [this](TMacroDesc &desc, Lexer &lexer)
    {
        ...
        if (currToken.mType != E_TOKEN_TYPE::NEWLINE)
        {
            ...
            const bool isStringificationOperator =
                currToken.mType == E_TOKEN_TYPE::STRINGIZE_OP;

            while ((currToken = lexer.GetNextToken()).mType !=
                   E_TOKEN_TYPE::NEWLINE)
            {
                if (E_TOKEN_TYPE::IDENTIFIER == currToken.mType &&
                    currToken.mRawView == desc.mName)
                {
                    desc.mValue.emplace_back(TToken{
                        E_TOKEN_TYPE::BLOB,
                ...
                if (E_TOKEN_TYPE::IDENTIFIER == currToken.mType &&
                    isStringificationOperator) // \note Validate argument of the
                                               // operator
                {
                    ...
                    if (std::find(
                            macroArgs.cbegin(),
                            macroArgs.cend(),
                            currToken.mRawView) == macroArgs.cend())
                    {
                        mOnErrorCallback(
                            {E_ERROR_TYPE::INCORRECT_STRINGIFY_OPERATOR_USAGE,
                             mpLexer->GetCurrLineIndex()});
                    }
                }
    ...
    default:
        mOnErrorCallback(
            {E_ERROR_TYPE::INVALID_MACRO_DEFINITION,
             mpLexer->GetCurrLineIndex()});
        break;
    }
    ...
    if (std::find_if(
            ...
            mSymTable.cend(),
            [&macroDesc](auto &&item)
            { return item.mName == macroDesc.mName; }) != mSymTable.cend())
    {
        mOnErrorCallback(
...
std::vector<TToken> Preprocessor::_expandMacroDefinition(
    const TMacroDesc &macroDesc,
    const TToken &idToken,
    const std::function<TToken()> &getNextTokenCallback) const TCPP_NOEXCEPT
{
        ...
        static const std::unordered_map<std::string, std::function<TToken()>>
            systemMacrosTable{
                {BuiltInDefines[0],
                 ...
                 {
                     return TToken{
                         E_TOKEN_TYPE::BLOB, std::to_string(idToken.mLineId)};
                 }}, // __LINE__
                {BuiltInDefines[1],
                 [&idToken]() {
                     return TToken{E_TOKEN_TYPE::BLOB, idToken.mRawView};
                 }}, // __VA_ARGS__
            };

        if (E_TOKEN_TYPE::CONCAT_OP ==
            mpLexer->PeekNextToken()
                .mType) // If an argument is stringized or concatenated, the
                        // prescan does not occur.
        {
            return {TToken{
                E_TOKEN_TYPE::BLOB,
                macroDesc.mName}}; // BLOB type is used instead of IDENTIFIER to
    ...
    {
        return {
            TToken{E_TOKEN_TYPE::BLOB, macroDesc.mName},
            currToken}; // \note Function like macro without brackets are is not
                        // expanded
    }
    ...
    while (true)
    {
        ...
        while ((currToken = getNextTokenCallback()).mType ==
               E_TOKEN_TYPE::SPACE) // \note skip space tokens
        {
            hasAnySpace = true;
        }
        ...
        if (E_TOKEN_TYPE::CLOSE_BRACKET == currToken.mType ||
            E_TOKEN_TYPE::COMMA == currToken.mType)
        {
            if (hasAnySpace)
            {
            ...
            {
                break; // \note There should be at least one space for empty
                       // argument
            }
        }
        else
    ...
    {
        mOnErrorCallback(
            {E_ERROR_TYPE::INCONSISTENT_MACRO_ARITY,
             mpLexer->GetCurrLineIndex()});
    }

    // \note execute macro's expansion
    std::vector<TToken> replacementList{
        macroDesc.mValue.cbegin(), macroDesc.mValue.cend()};
    const auto &argsList = macroDesc.mArgsNames;

    std::string replacementValue;

    for (size_t currArgIndex = 0; currArgIndex < processingTokens.size();
         ++currArgIndex)
    {
        const bool variadics = macroDesc.mVariadic &&
                               currArgIndex == macroDesc.mArgsNames.size() - 1;
        const std::string &currArgName =
            variadics ? argsList.back() : argsList[currArgIndex];
...
/*std::vector<TToken> Preprocessor::_expandMacroArg(const TMacroDesc& macroDesc,
const TToken& idToken, const std::function<TToken()>& getNextTokenCallback)
const TCPP_NOEXCEPT
{
}*/

void Preprocessor::_expect(
    const E_TOKEN_TYPE &expectedType,
    const E_TOKEN_TYPE &actualType) const TCPP_NOEXCEPT
{
...
void Preprocessor::_processInclusion() TCPP_NOEXCEPT
{
    ...
    if (currToken.mType != E_TOKEN_TYPE::LESS &&
        currToken.mType != E_TOKEN_TYPE::QUOTES)
    {
        while ((currToken = mpLexer->GetNextToken()).mType ==
               E_TOKEN_TYPE::NEWLINE)
        ...
        mOnErrorCallback(
            {E_ERROR_TYPE::INVALID_INCLUDE_DIRECTIVE,
             mpLexer->GetCurrLineIndex()});
        return;
    }
    ...
    while (true)
    {
        if ((currToken = mpLexer->GetNextToken()).mType ==
                E_TOKEN_TYPE::QUOTES ||
        ...
        {
            mOnErrorCallback(
                {E_ERROR_TYPE::UNEXPECTED_END_OF_INCLUDE_PATH,
                 mpLexer->GetCurrLineIndex()});
            break;
        }
    ...
    if (E_TOKEN_TYPE::NEWLINE != currToken.mType &&
        E_TOKEN_TYPE::END != currToken.mType)
    {
        mOnErrorCallback(
            {E_ERROR_TYPE::UNEXPECTED_TOKEN, mpLexer->GetCurrLineIndex()});
    }
    ...
    {
        mpLexer->PushStream(
            std::move(mOnIncludeCallback(path, isSystemPathInclusion)));
    }
}
...
Preprocessor::TIfStackEntry Preprocessor::_processIfConditional() TCPP_NOEXCEPT
{
    ...
    // \note IsDisabledBlockProcessed is used to inherit disabled state for
    // nested blocks
    return TIfStackEntry(
        static_cast<bool>(!_evaluateExpression(expressionTokens)),
        IsParentBlockActive(mConditionalBlocksStack));
}
...
Preprocessor::_processIfdefConditional() TCPP_NOEXCEPT
{
    ...
    // \note IsParentBlockActive is used to inherit disabled state for nested
    // blocks
    ...
    TCPP_NOEXCEPT
{
    ...
    {
        mOnErrorCallback(
            {E_ERROR_TYPE::ANOTHER_ELSE_BLOCK_FOUND,
             mpLexer->GetCurrLineIndex()});
        return;
    }

    currStackEntry.mShouldBeSkipped = currStackEntry.mHasIfBlockBeenEntered ||
    ...
    TCPP_NOEXCEPT
{
    ...
    {
        mOnErrorCallback(
            {E_ERROR_TYPE::ELIF_BLOCK_AFTER_ELSE_FOUND,
             mpLexer->GetCurrLineIndex()});
        return;
    }
    ...
    if (!currStackEntry.mShouldBeSkipped)
        currStackEntry.mHasIfBlockBeenEntered = true;
}
    ...
    const std::vector<TToken> &exprTokens) const TCPP_NOEXCEPT
{
    ...
    auto evalPrimary = [this, &tokens]()
    {
        ...
        case E_TOKEN_TYPE::IDENTIFIER:
        {
            ...
            {
                /// \note Lexer for now doesn't support numbers recognition so
                /// numbers are recognized as identifiers too
            ...
            else
            {
                if (it->mArgsNames.empty())
                {
                    return _evaluateExpression(
                        it->mValue); /// simple macro replacement
                }

                /// \note Macro function call so we should firstly expand that
                /// one
                auto currTokenIt = tokens.cbegin();
                return _evaluateExpression(_expandMacroDefinition(
                    *it,
                    identifierToken,
                    [&currTokenIt] { return *currTokenIt++; }));
            }
    ...
    auto evalUnary = [&tokens, &evalPrimary]()
    {
        ...
        TToken currToken;
        while ((currToken = tokens.front()).mType == E_TOKEN_TYPE::NOT ||
        ...
        // odd number of NOTs: true ^ false = true (!false), true ^ true = false
        // (!true)
    ...
    auto evalMultiplication = [&tokens, &evalUnary]()
    {
               ...
               currToken.mType == E_TOKEN_TYPE::SLASH)
        {
            ...
            case E_TOKEN_TYPE::SLASH:
                ...
                result = secondOperand ? (result / secondOperand)
                                       : 0 /* division by zero is considered as
...
bool Preprocessor::_shouldTokenBeSkipped() const TCPP_NOEXCEPT
{
    return !mConditionalBlocksStack.empty() &&
           (mConditionalBlocksStack.top().mShouldBeSkipped ||
            !mConditionalBlocksStack.top().mIsParentBlockActive);
}
